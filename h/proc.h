...
#define PRLOCK      '\008'        /* process is on lock queue*/
...
struct	pentry	{
    ...
    int lockid; /* ldes of lock in whose wait queue the process is currently blocked */
    int lock_wait_ret;      /* used to implement the deleted requirement - wait return from locks */
    long lock_req_time; /* lock request time used to check the 10 ms gap when releasing */
    int lock_proc_type[NLOCKS]; /* different locks hold by the process */
};
